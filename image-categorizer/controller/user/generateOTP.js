    ...
    async handleRequest(req, res){
        ...
        try{
            ...
            if(userData.rows.length === 0){
                return this.helper.writeResponse({msg : "The entered Email already exist!" ,code : 404},{status : false},res);
            }
            else{
                const transporter = await this.userUtility.initNodeMailer();
                await this.userUtility.createOTP(email,transporter);
                return this.helper.writeResponse(null,{"msg" : "OTP has been sent to your Gmail","status" : true},res);
            }
        }
        catch(err){
            console.log(err);
